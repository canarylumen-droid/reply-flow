  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-white transition-colors">
      ...
      <header className="border-b border-gray-100 dark:border-white/5 bg-gray-50/80 dark:bg-zinc-950">
        <div className="max-w-2xl mx-auto px-5 sm:px-8 pt-10 pb-10">

          {/* Breadcrumb */}
          <a href="/blog" className="inline-flex items-center gap-1.5 text-[13px] text-gray-400 hover:text-primary transition-colors font-medium mb-7">
          ...
          {/* Title — bold but not black */}
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-syne font-bold leading-[1.18] text-gray-900 dark:text-white mb-5">
            {post.title}
          </h1>

          {/* Description */}
          {post.description && (
            <p className="text-[15px] sm:text-base text-gray-500 dark:text-gray-400 leading-relaxed mb-7 font-normal">
              {post.description}
            </p>
          )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 pt-6 border-t border-gray-200 dark:border-white/8">
            ...
            <div className="flex items-center gap-2.5 flex-1">
              <div className="w-9 h-9 bg-primary rounded-xl flex items-center justify-center text-white font-syne font-bold text-[11px] shadow-md shadow-primary/20 shrink-0">
                RF
              </div>
              <div>
                <p className="text-[13px] font-semibold text-gray-900 dark:text-white leading-tight">ReplyFlow Agency</p>
                <p className="text-[11px] text-gray-400">replyflow.pro</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handleCopy}
                title="Copy link"
                className={`w-8 h-8 flex items-center justify-center rounded-lg border transition-all ${copied ? 'border-emerald-400 text-emerald-500' : 'border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 hover:text-primary hover:border-primary/40 dark:hover:text-primary'}`}
              >
      ...
      <motion.article
        ...
        className="max-w-2xl mx-auto px-5 sm:px-8 py-12"
      >
        <div
          ...
          <div className="flex items-center gap-2">
            {shareLinks.map(s => (
              <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" title={s.label}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-white/10 text-[12px] font-medium text-gray-600 dark:text-gray-300 hover:text-primary hover:border-primary/30 transition-all gap-1.5">
